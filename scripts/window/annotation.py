...
class AnnotationSubWindow(Window):
    def __init__(self, name, img, x, y, points, offset, sz = 256, n = 3) -> None:
    ...
    def update(self) -> None:
        ...
        if points:        
            ...
            self.img = cv2.addWeighted( self.img, 0.25, self.org_img, 0.75, 0.0)
            cv2.polylines( self.img, [points], False, (0, 0, 255), 10)
        '''
        self.img = self.img[
            self.y - self.padding: self.y + self.padding,
            self.x - self.padding: self.x + self.padding
        ]
        print(self.img.shape)
        print(self.img)
        

    # Check if the sub window has been destroyed (i.e, closed)
    def closed(self):
...
class AnnotationWindow(Window):
    ...
    def click_handler(self, event, x, y, flags, params):
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            self.sub_window = AnnotationSubWindow("Sub window", self.img, x, y, self.points, self.offset)
            
    # Handle keybord inputs
    def key_handler(self, key) -> None:
        if self.sub_window is not None:
            #if not print(self.sub_window.closed():
            self.sub_window.key_handler(key)
            #else:
            #    self.sub_window = None
        else:
            ...
            for _ in range(samples):
                ...
                if cntNoneZeros == 0:
                    ...
                    if counter['white'] > samples * ratio:
                        continue
                    counter['white'] += 1
                sub_img = self.org_img[y: y + sz, x: x + sz]
